...
void copy_flocker_states_to_graphics_objects() {
	...
	for (int i = 0; i < num_flockers; i++) {
		xyz_positions[i] = glm::vec3(flocker_array[i]->position.x, flocker_array[i]->position.y,
									 flocker_array[i]->position.z);
		quat_orientations[i] = glm::normalize(
				glm::quat(glm::vec3(drand48() * 360.0, drand48() * 360.0, drand48() * 360.0)));
	}
}
...
void bullet_add_dynamic_objects() {
	...
	for (int i = 0; i < num_flockers; i++) {
		...
		btDefaultMotionState *motionstate = new btDefaultMotionState(btTransform(
				btQuaternion(quat_orientations[i].x, quat_orientations[i].y, quat_orientations[i].z,
							 quat_orientations[i].w),
...
void initialize_bullet_simulator() {
	...
	bullet_dynamicsWorld = new btDiscreteDynamicsWorld(bullet_dispatcher, bullet_broadphase, bullet_solver,
													   bullet_collisionConfiguration);
