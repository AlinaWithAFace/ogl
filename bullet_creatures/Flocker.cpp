...
bool Flocker::compute_alignment_force() {
	...
	for (j = index + 1; j < flocker_squared_distance.size(); j++)
		if (flocker_squared_distance[index][j] >= min_squared_alignment_distance &&
			...
			percent = (flocker_squared_distance[index][j] - max_squared_alignment_distance) *
					  inv_range_squared_alignment_distance;
...
bool Flocker::compute_cohesion_force() {
	...
	for (j = index + 1; j < flocker_squared_distance.size(); j++)
		if (flocker_squared_distance[index][j] >= min_squared_cohesion_distance &&
			...
			percent = (flocker_squared_distance[index][j] - max_squared_cohesion_distance) *
					  inv_range_squared_cohesion_distance;
			F = 0.5 + -0.5 * cos(percent * 2.0 * M_PI);

			// set force direction

			direction = (float) F *
						glm::normalize(flocker_array[j]->position - position);   // opposite direction of separation
